...
static void write_unknown_chunks(png_structrp png_ptr,
  ...
  if (info_ptr->unknown_chunks_num != 0) {
    ...
    for (up = info_ptr->unknown_chunks;
         up < info_ptr->unknown_chunks + info_ptr->unknown_chunks_num; ++up)
...
void PNGAPI png_write_info(png_structrp png_ptr, png_const_inforp info_ptr) {
  ...
  if ((info_ptr->valid & PNG_INFO_pCAL) != 0)
    png_write_pCAL(png_ptr, info_ptr->pcal_purpose, info_ptr->pcal_X0,
                   info_ptr->pcal_X1, info_ptr->pcal_type,
                   info_ptr->pcal_nparams, info_ptr->pcal_units,
                   info_ptr->pcal_params);
#endif

#ifdef PNG_WRITE_sCAL_SUPPORTED
  if ((info_ptr->valid & PNG_INFO_sCAL) != 0)
    png_write_sCAL_s(png_ptr, (int)info_ptr->scal_unit, info_ptr->scal_s_width,
...
PNG_FUNCTION(png_structp, PNGAPI png_create_write_struct,
             (png_const_charp user_png_ver, png_voidp error_ptr,
              png_error_ptr error_fn, png_error_ptr warn_fn),
             PNG_ALLOCATED) {
#ifndef PNG_USER_MEM_SUPPORTED
  png_structrp png_ptr = png_create_png_struct(
      user_png_ver, error_ptr, error_fn, warn_fn, NULL, NULL, NULL);
#else
  return png_create_write_struct_2(user_png_ver, error_ptr, error_fn, warn_fn,
                                   NULL, NULL, NULL);
}
...
void PNGAPI png_write_row(png_structrp png_ptr, png_const_bytep row) {
  ...
  png_debug2(1, "in png_write_row (row %u, pass %d)", png_ptr->row_number,
             png_ptr->pass);
  ...
  if (method == PNG_FILTER_TYPE_BASE) {
    ...
    if (png_ptr->row_buf != NULL) {
      ...
      if (png_ptr->try_row == NULL)
        png_ptr->try_row =
            png_voidcast(png_bytep, png_malloc(png_ptr, buf_size));
...
void PNGAPI png_write_png(png_structrp png_ptr, png_inforp info_ptr,
...
static int png_write_image_16bit(png_voidp argument) {
  png_image_write_control *display =
      png_voidcast(png_image_write_control *, argument);
  png_imagep image = display->image;
  png_structrp png_ptr = image->opaque->png_ptr;

  png_const_uint_16p input_row =
      png_voidcast(png_const_uint_16p, display->first_row);
...
static void png_image_set_PLTE(png_image_write_control *display) {
  ...
  int entries =
      image->colormap_entries > 256 ? 256 : (int)image->colormap_entries;
  ...
  for (i = num_trans = 0; i < entries; ++i) {
    ...
    if ((format & PNG_FORMAT_FLAG_LINEAR) != 0) {
      ...
      if ((channels & 1) != 0) /* no alpha */
      {
        ...
        {
          palette[i].blue =
              (png_byte)PNG_sRGB_FROM_LINEAR(255 * entry[(2 ^ bgr)]);
      ...
      else /* alpha */
      {
        ...
        {
          palette[i].blue =
              png_unpremultiply(entry[afirst + (2 ^ bgr)], alpha, reciprocal);
          palette[i].green =
              png_unpremultiply(entry[afirst + 1], alpha, reciprocal);
          palette[i].red =
              png_unpremultiply(entry[afirst + bgr], alpha, reciprocal);
        }
...
static int png_image_write_main(png_voidp argument) {
  ...
  if ((format & PNG_FORMAT_FLAG_COLORMAP) != 0) {
    ...
    png_set_IHDR(
        png_ptr, info_ptr, image->width, image->height, write_16bit ? 16 : 8,
  ...
  if ((format &
       ~(png_uint_32)(PNG_FORMAT_FLAG_COLOR | PNG_FORMAT_FLAG_LINEAR |
...
int PNGAPI png_image_write_to_memory(
  ...
  if (image != NULL && image->version == PNG_IMAGE_VERSION) {
    ...
    return png_image_error(
        image, "png_image_write_to_memory: incorrect PNG_IMAGE_VERSION");
...
#ifdef PNG_SIMPLIFIED_WRITE_STDIO_SUPPORTED
int PNGAPI png_image_write_to_stdio(png_imagep image, FILE *file,
  ...
  if (image != NULL && image->version == PNG_IMAGE_VERSION) {
    ...
    return png_image_error(
        image, "png_image_write_to_stdio: incorrect PNG_IMAGE_VERSION");
...
int PNGAPI png_image_write_to_file(png_imagep image, const char *file_name,
  ...
  if (image != NULL && image->version == PNG_IMAGE_VERSION) {
    ...
    return png_image_error(
        image, "png_image_write_to_file: incorrect PNG_IMAGE_VERSION");
