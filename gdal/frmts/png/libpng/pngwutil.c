...
void PNGAPI png_write_chunk_start(png_structrp png_ptr,
 ...
 * functions instead.
 */
static void png_write_complete_chunk(png_structrp png_ptr,
...
void PNGAPI png_write_chunk(png_structrp png_ptr, png_const_bytep chunk_string,
                            png_const_bytep data, size_t length) {
...
static int png_deflate_claim(png_structrp png_ptr, png_uint_32 owner,
...

  {
    ...
    if ((png_ptr->flags & PNG_FLAG_ZSTREAM_INITIALIZED) != 0)
      ...
      ret = deflateInit2(&png_ptr->zstream, level, method, windowBits, memLevel,
                         strategy);
...
static int png_text_compress(png_structrp png_ptr, png_uint_32 chunk_name,
      ...
      if (png_ptr->zstream.avail_out == 0) {
        ...
        if (next == NULL) {
          next = png_voidcast(
              ...
              png_malloc_base(png_ptr, PNG_COMPRESSION_BUFFER_SIZE(png_ptr)));
...
png_compress_IDAT(png_structrp png_ptr, png_const_bytep input,
  ...
  if (png_ptr->zowner != png_IDAT) {
    ...
    if (png_ptr->zbuffer_list == NULL) {
      png_ptr->zbuffer_list = png_voidcast(
          png_compression_bufferp,
...
png_write_sBIT(png_structrp png_ptr, png_const_color_8p sbit, int color_type) {
  ...
  if ((color_type & PNG_COLOR_MASK_COLOR) != 0) {
    ...
    maxbits = (png_byte)(
        color_type == PNG_COLOR_TYPE_PALETTE ? 8 : png_ptr->usr_bit_depth);

    if (sbit->red == 0 || sbit->red > maxbits || sbit->green == 0 ||
        sbit->green > maxbits || sbit->blue == 0 || sbit->blue > maxbits) {
...
#endif
    {
      png_warning(png_ptr, "Ignoring attempt to write 16-bit bKGD chunk "
                           "when bit_depth is 8");
...
png_write_iTXt(png_structrp png_ptr, int compression, png_const_charp key,
               png_const_charp lang, png_const_charp lang_key,
...
png_write_pCAL(png_structrp png_ptr, png_charp purpose, png_int_32 X0,
  ...
  params_len = (png_size_tp)png_malloc(
      png_ptr,
...
png_write_start_row(png_structrp png_ptr) {
  ...
  if (png_ptr->interlaced != 0) {
    if ((png_ptr->transformations & PNG_INTERLACE) == 0) {
      ...
      png_ptr->usr_width =
          (png_ptr->width + png_pass_inc[0] - 1 - png_pass_start[0]) /
          png_pass_inc[0];
    }
  ...
  if (png_ptr->interlaced != 0) {
    ...
    if ((png_ptr->transformations & PNG_INTERLACE) != 0) {
      ...
      do {
        ...
        png_ptr->usr_width = (png_ptr->width + png_pass_inc[png_ptr->pass] - 1 -
...
png_do_write_interlace(png_row_infop row_info, png_bytep row, int pass) {
  ...
  if (pass < 6) {
    ...
    /* Set new row width */
    row_info->width =
        (row_info->width + png_pass_inc[pass] - 1 - png_pass_start[pass]) /
        png_pass_inc[pass];
...
png_setup_sub_row(png_structrp png_ptr, png_uint_32 bpp, size_t row_bytes,
                  size_t lmins) {
  png_bytep rp, dp, lp;
  size_t i;
  size_t sum = 0;
...
static size_t /* PRIVATE */
png_setup_avg_row(png_structrp png_ptr, png_uint_32 bpp, size_t row_bytes,
                  size_t lmins) {
...
static size_t /* PRIVATE */
png_setup_paeth_row(png_structrp png_ptr, png_uint_32 bpp, size_t row_bytes,
                    size_t lmins) {
