...
static png_bytep png_read_buffer(png_structrp png_ptr,
                                 png_alloc_size_t new_size, int warn) {
...
#if ZLIB_VERNUM >= 0x1290 && defined(PNG_SET_OPTION_SUPPORTED) &&              \
    defined(PNG_IGNORE_ADLER32)
...
static int png_inflate(png_structrp png_ptr, png_uint_32 owner, int finish,
                       /* INPUT: */ png_const_bytep input,
                       png_uint_32p input_size_ptr,
                       /* OUTPUT: */ png_bytep output,
  ...
  if (png_ptr->zowner == owner) /* Else not claimed */
  {
    ...
    do {
       ...
       */
      ret = PNG_INFLATE(png_ptr, avail_out > 0
                                     ? Z_NO_FLUSH
 ...
 */
static int png_decompress_chunk(
    png_structrp png_ptr, png_uint_32 chunklength, png_uint_32 prefix_size,
  ...
  if (limit >= prefix_size + (terminate != 0)) {
    ...
    if (ret == Z_OK) {
      ...
      ret =
          png_inflate(png_ptr, png_ptr->chunk_name, 1 /*finish*/,
      ...
      if (ret == Z_STREAM_END) {
        ...
        if (inflateReset(&png_ptr->zstream) == Z_OK) {
          ...
          png_alloc_size_t buffer_size =
              prefix_size + new_size + (terminate != 0);
          png_bytep text =
              png_voidcast(png_bytep, png_malloc_base(png_ptr, buffer_size));
 ...
 */
static int png_inflate_read(png_structrp png_ptr, png_bytep read_buffer,
                            uInt read_size, png_uint_32p chunk_bytes,
                            png_bytep next_out, png_alloc_size_t *out_size,
                            int finish) {
...
png_handle_cHRM(png_structrp png_ptr, png_inforp info_ptr, png_uint_32 length) {
  ...
  if (xy.whitex == PNG_FIXED_ERROR || xy.whitey == PNG_FIXED_ERROR ||
      xy.redx == PNG_FIXED_ERROR || xy.redy == PNG_FIXED_ERROR ||
      xy.greenx == PNG_FIXED_ERROR || xy.greeny == PNG_FIXED_ERROR ||
      xy.bluex == PNG_FIXED_ERROR || xy.bluey == PNG_FIXED_ERROR) {
      ...
      if (keyword_length + 1 < read_length &&
        ...
        if (png_inflate_claim(png_ptr, png_iCCP) == Z_OK) {
          ...
          (void)png_inflate_read(
              png_ptr, local_buffer, (sizeof local_buffer), &length,
              profile_header, &size,
          ...
          if (size == 0) {
            ...
            if (png_icc_check_length(png_ptr, &png_ptr->colorspace, keyword,
                                     profile_length) != 0) {
              /* The length is apparently ok, so we can check the 132
               * byte header.
               */
              if (png_icc_check_header(png_ptr, &png_ptr->colorspace, keyword,
                                       profile_length, profile_header,
                ...
                png_bytep profile =
                    png_read_buffer(png_ptr, profile_length, 2 /*silent*/);
                ...
                if (profile != NULL) {
                   ...
                   */
                  if (size == 0) {
                    if (png_icc_check_tag_table(png_ptr, &png_ptr->colorspace,
                      ...
                      (void)png_inflate_read(
                          png_ptr, local_buffer, (sizeof local_buffer), &length,
                          profile + (sizeof profile_header) + 12 * tag_count,
                          &size, 1 /*finish*/);
                      ...
                      /* But otherwise allow extra data: */
                        ...
                        png_icc_set_sRGB(png_ptr, &png_ptr->colorspace, profile,
                        ...
                        if (info_ptr != NULL) {
                          ...
                          info_ptr->iccp_name = png_voidcast(
                              char *,
                              png_malloc_base(png_ptr, keyword_length + 1));
...
png_handle_tRNS(png_structrp png_ptr, png_inforp info_ptr, png_uint_32 length) {
  ...
  if (png_ptr->color_type == PNG_COLOR_TYPE_GRAY) {
    ...
    if (length > (unsigned int)png_ptr->num_palette ||
        length > (unsigned int)PNG_MAX_PALETTE_LENGTH || length == 0) {
      png_crc_finish(png_ptr, length);
      png_chunk_benign_error(png_ptr, "invalid");
      return;
    }
...
png_handle_pCAL(png_structrp png_ptr, png_inforp info_ptr, png_uint_32 length) {
  ...
  params = png_voidcast(
      png_charpp, png_malloc_warn(png_ptr, nparams * (sizeof(png_charp))));
...
png_handle_sCAL(png_structrp png_ptr, png_inforp info_ptr, png_uint_32 length) {
  ...
  if (png_check_fp_number((png_const_charp)buffer, length, &state, &i) == 0 ||
    ...
    if (png_check_fp_number((png_const_charp)buffer, length, &state, &i) == 0 ||
      ...
      png_set_sCAL_s(png_ptr, info_ptr, buffer[0], (png_charp)buffer + 1,
                     (png_charp)buffer + heighti);
  }
}
#endif
...
png_handle_zTXt(png_structrp png_ptr, png_inforp info_ptr, png_uint_32 length) {
  ...
  for (keyword_length = 0;
       keyword_length < length && buffer[keyword_length] != 0; ++keyword_length)
    /* Empty loop to find end of name */;
...
png_handle_iTXt(png_structrp png_ptr, png_inforp info_ptr, png_uint_32 length) {
  ...
  /* First the keyword. */
  for (prefix_length = 0; prefix_length < length && buffer[prefix_length] != 0;
       ++prefix_length)
    /* Empty loop */;
...
png_handle_unknown(png_structrp png_ptr, png_inforp info_ptr,
  ...
  if (png_ptr->read_user_chunk_fn != NULL) {
    if (png_cache_unknown_chunk(png_ptr, length) != 0) {
      ...
      int ret =
          (*(png_ptr->read_user_chunk_fn))(png_ptr, &png_ptr->unknown_chunk);
...
png_do_read_interlace(
    ...
    png_uint_32
        transformations /* Because these may affect the byte layout */) {
