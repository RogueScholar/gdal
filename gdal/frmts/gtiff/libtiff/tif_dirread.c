...
static enum TIFFReadDirEntryErr
TIFFReadDirEntryArray(TIFF *tif, TIFFDirEntry *direntry, uint32 *count,
                      uint32 desttypesize, void **value);
static enum TIFFReadDirEntryErr
  ...
  while (already_read < size) {
    ...
    if (new_dest == NULL) {
      TIFFErrorExt(tif->tif_clientdata, tif->tif_name,
                   ...
                   "TIFFReadDirEntryArray", (long)1,
                   (long)(already_read + to_read));
...
static enum TIFFReadDirEntryErr
TIFFReadDirEntryArrayWithLimit(TIFF *tif, TIFFDirEntry *direntry, uint32 *count,
                               uint32 desttypesize, void **value,
                               uint64 maxcount) {
  int typesize;
  uint32 datasize;
  ...
  *count = (uint32)target_count64;
  ...
  if (!(tif->tif_flags & TIFF_BIGTIFF)) {
    if (datasize <= 4)
      ...
      if (isMapped(tif))
        err =
            TIFFReadDirEntryData(tif, (uint64)offset, (tmsize_t)datasize, data);
      else
        err = TIFFReadDirEntryDataAndRealloc(tif, (uint64)offset,
...
static enum TIFFReadDirEntryErr
TIFFReadDirEntryCheckedDouble(TIFF *tif, TIFFDirEntry *direntry,
                              double *value) {
  assert(sizeof(double) == 8);
  assert(sizeof(uint64) == 8);
...
TIFFReadDirEntryData(TIFF *tif, uint64 offset, tmsize_t size, void *dest) {
  ...
  if (!isMapped(tif)) {
    ...
    size_t ma, mb;
    ma = (size_t)offset;
    if ((uint64)ma != offset || ma > (~(size_t)0) - (size_t)size) {
      return TIFFReadDirEntryErrIo;
    }
    mb = ma + size;
...
static void TIFFReadDirEntryOutputErr(TIFF *tif, enum TIFFReadDirEntryErr err,
  ...
  if (!recover) {
    ...
    case TIFFReadDirEntryErrAlloc:
      TIFFErrorExt(tif->tif_clientdata, module,
                   "Out of memory reading of \"%s\"", tagname);
      break;
    default:
...
int TIFFReadDirectory(TIFF *tif) {
  ...
  if (!dircount) {
    TIFFErrorExt(tif->tif_clientdata, module,
                 "Failed to read directory at offset " TIFF_UINT64_FORMAT,
                 nextdiroff);
    return 0;
  }
    ...
    if (!dp->tdir_ignore) {
         ...
         */
        {
          ...
          if (err != TIFFReadDirEntryErrOk) {
            ...
            TIFFReadDirEntryOutputErr(
                tif, err, module, fip ? fip->field_name : "unknown tagname", 0);
            goto bad;
          }
      ...
      case TIFFTAG_TRANSFERFUNCTION: {
        ...
        if (err != TIFFReadDirEntryErrOk) {
          fip = TIFFFieldWithTag(tif, dp->tdir_tag);
          TIFFReadDirEntryOutputErr(
              tif, err, module, fip ? fip->field_name : "unknown tagname", 1);
        } else {
...
int TIFFReadCustomDirectory(TIFF *tif, toff_t diroff,
  ...
  if (!dircount) {
    TIFFErrorExt(
        tif->tif_clientdata, module,
        "Failed to read custom directory at offset " TIFF_UINT64_FORMAT,
        diroff);
    return 0;
  }
  TIFFFreeDirectory(tif);
...
static int EstimateStripByteCounts(TIFF *tif, TIFFDirEntry *dir,
  ...
  td->td_stripbytecount_p = (uint64 *)_TIFFCheckMalloc(
      tif, td->td_nstrips, sizeof(uint64), "for \"StripByteCounts\" array");
  ...
  if (td->td_compression != COMPRESSION_NONE) {
    ...
    if (td->td_stripoffset_p[strip] >
        TIFF_UINT64_MAX - td->td_stripbytecount_p[strip])
      return -1;
...
static int TIFFCheckDirOffset(TIFF *tif, uint64 diroff) {
  ...
  if (tif->tif_dirlist == NULL || tif->tif_dirnumber > tif->tif_dirlistsize) {
     ...
     */
    new_dirlist =
        (uint64 *)_TIFFCheckRealloc(tif, tif->tif_dirlist, tif->tif_dirnumber,
                                    2 * sizeof(uint64), "for IFD list");
    if (!new_dirlist)
      return 0;
...
static int CheckDirCount(TIFF *tif, TIFFDirEntry *dir, uint32 count) {
  if ((uint64)count > dir->tdir_count) {
    ...
    TIFFWarningExt(tif->tif_clientdata, tif->tif_name,
                   "incorrect count for field \"%s\" (" TIFF_UINT64_FORMAT
                   ", expecting %u); tag ignored",
                   fip ? fip->field_name : "unknown tagname", dir->tdir_count,
                   count);
    return (0);
...
static uint16 TIFFFetchDirectory(TIFF *tif, uint64 diroff, TIFFDirEntry **pdir,
  ...
  dir = (TIFFDirEntry *)_TIFFCheckMalloc(tif, dircount16, sizeof(TIFFDirEntry),
...
static int TIFFFetchStripThing(TIFF *tif, TIFFDirEntry *dir, uint32 nstrips,
  ...
  if (dir->tdir_count < (uint64)nstrips) {
    ...
    resizeddata = (uint64 *)_TIFFCheckMalloc(tif, nstrips, sizeof(uint64),
                                             "for strip array");
    if (resizeddata == 0) {
      _TIFFfree(data);
      return (0);
...
static void allocChoppedUpStripArrays(TIFF *tif, uint32 nstrips,
  ...
  newcounts = (uint64 *)_TIFFCheckMalloc(
      tif, nstrips, sizeof(uint64), "for chopped \"StripByteCounts\" array");
  ...
  for (i = 0; i < td->td_nstrips; i++) {
    if (i == td->td_nstrips - 1) {
      if (TIFFGetStrileByteCount(tif, i) <
          TIFFVStripSize64(tif, td->td_imagelength - i * td->td_rowsperstrip)) {
        return;
      }
    } else {
...
static uint64 _TIFFUnsanitizedAddUInt64AndInt(uint64 a, int b) { return a + b; }
...
#undef IO_CACHE_PAGE_SIZE
  ...
  if (strile + iStartBefore < 0)
    iStartBefore = -strile;
  for (i = iStartBefore; (uint32)(strile + i) < arraySize &&
                         _TIFFUnsanitizedAddUInt64AndInt(
                             nOffset, (i + 1) * sizeofvalint) <= nOffsetEndPage;
       ++i) {
    if (dirent->tdir_type == TIFF_SHORT) {
      uint16 val;
...
static int _TIFFFetchStrileValue(TIFF *tif, uint32 strile, TIFFDirEntry *dirent,
                                 uint64 **parray) {
  ...
  if (strile >= td->td_stripoffsetbyteallocsize) {
    ...
    if (td->td_stripoffsetbyteallocsize == 0 && td->td_nstrips < 1024 * 1024) {
...
#endif
    offsetArray = (uint64 *)(_TIFFrealloc(td->td_stripoffset_p, nArraySize));
    bytecountArray =
        (uint64 *)(_TIFFrealloc(td->td_stripbytecount_p, nArraySize));
    if (offsetArray)
...
uint64 TIFFGetStrileOffsetWithErr(TIFF *tif, uint32 strile, int *pbErr) {
  ...
  return _TIFFGetStrileOffsetOrByteCountValue(
      tif, strile, &(td->td_stripoffset_entry), &(td->td_stripoffset_p), pbErr);
}
...
static int _TIFFFillStrilesInternal(TIFF *tif, int loadStripByteCount) {
  ...
  if (loadStripByteCount &&
      !TIFFFetchStripThing(tif, &(td->td_stripbytecount_entry), td->td_nstrips,
