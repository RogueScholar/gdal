...
uint32_t hilbert(const NodeItem &r, uint32_t hilbertMax,
                 const NodeItem &extent) {
  uint32_t x = static_cast<uint32_t>(floor(
      hilbertMax * ((r.minX + r.maxX) / 2 - extent.minX) / extent.width()));
  uint32_t y = static_cast<uint32_t>(floor(
      hilbertMax * ((r.minY + r.maxY) / 2 - extent.minY) / extent.height()));
...
void hilbertSort(std::vector<std::shared_ptr<Item>> &items) {
  NodeItem extent =
      std::accumulate(items.begin(), items.end(), NodeItem::create(0),
                      [](NodeItem a, std::shared_ptr<Item> b) {
                        a.expand(b->nodeItem);
                        return a;
                      });
  std::sort(items.begin(), items.end(),
            [&extent](std::shared_ptr<Item> a, std::shared_ptr<Item> b) {
...
NodeItem calcExtent(const std::vector<NodeItem> &nodes) {
  NodeItem extent =
      std::accumulate(nodes.begin(), nodes.end(), NodeItem::create(0),
...
void hilbertSort(std::vector<NodeItem> &items) {
  ...
  std::sort(items.begin(), items.end(),
            [&extent](const NodeItem &a, const NodeItem &b) {
...
PackedRTree::search(double minX, double minY, double maxX, double maxY) const {
  ...
  while (queue.size() != 0) {
    ...
    uint64_t end = std::min(static_cast<uint64_t>(nodeIndex + _nodeSize),
...
#if !CPL_IS_LSB
  // Note: we should normally revert endianness after writing, but as we no
  // longer use the data structures this is not needed.
